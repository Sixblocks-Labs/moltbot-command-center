...
const FILE_PATH = '/home/clawdbot/.moltbot/arginfra.md';
...
function parseArgInfra(md: string): Entry[] {
...
export async function GET() {
  try {
    ...
    const entries = parseArgInfra(content);
    return NextResponse.json({ path: FILE_PATH, entries });
  } catch (err: any) {
    if (err?.code === 'ENOENT') {
      return NextResponse.json({ path: FILE_PATH, entries: [] });
    }
    return NextResponse.json({ error: 'Failed to read arginfra' }, { status: 500 });
  }
}
